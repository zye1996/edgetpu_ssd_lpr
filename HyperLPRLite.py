...
class LP_Detector():
    ...
    def __init__(self, model_detection, label_path, threshold=0.2):
        self.label = self.load_labels(label_path)
        self.interpreter = make_interpreter(model_detection)
    ...
    def predict(self, image):
        ...
        cropped_images = [(np.array(image.crop(box=obj.bbox).convert('RGB')),
                              [obj.bbox.xmin, obj.bbox.ymin, obj.bbox.width, obj.bbox.height]) for obj in objs]
...
class LP_Recognizer():
    ...
    def __init__(self, model_recognition):
        self.interpreter = make_interpreter(model_recognition)
        self.interpreter.allocate_tensors()
        self.input_index = self.interpreter.get_input_details()[0]['index']
        self.output_index = self.interpreter.get_output_details()[0]['index']
        self.mean, self.std_dev = self.interpreter.get_output_details()[0]['quantization']

    def predict(self, img):
        self.interpreter.set_tensor(self.input_index, img)
        self.interpreter.invoke()
        result = self.interpreter.get_tensor(self.output_index)
        result = (result - self.mean) / self.std_dev
        return result


class LPR():
    def __init__(self, model_detection, model_finemapping, model_seq_rec, ssd=True, tpu=False, cnn=True):
        if ssd:
            self.ssd = True
            self.ssd_detector = LP_Detector(model_detection, label_path="inference/inference_label.txt")
        else:
            self.ssd = False
        ...
        if cnn:
            self.plate_size = (160, 40)
            if tpu:
                self.modelSeqRec = LP_Recognizer(model_seq_rec)
            else:
                self.modelSeqRec = self.model_seq_rec_cnn(model_seq_rec)
        else:
            self.plate_size = (164, 48)
    ...
    def detectPlateRough_ssd(self, image_rgb):
        pass
    ...
    def model_seq_rec_cnn(self, model_path):
    ...
    def finemappingVertical(self, image, rect):
        ...
        cv2.imwrite("images_rec/ocr_images/7.jpg", cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
        return image, rect

    def recognizeOne(self, src):
        x_tempx = src
        x_temp = cv2.resize(x_tempx, self.plate_size)
    ...
    def SimpleRecognizePlateByE2E(self,image):
        if self.ssd:
            images = self.ssd_detector.predict(image)
        else:
...
if __name__ == '__main__':
    ...
    model = LPR("export_graph/edgetpu_models/detection_model_edgetpu.tflite",
    ...
    '''
    img = Image.open("images_rec/1_resized.jpg")
    img_detector = LP_Detector("export_graph/detection_model_ssd_300/detection_model_edgetpu.tflite",
                               label_path="inference/inference_label.txt")
    cropped_image = img_detector.predict(img)
    for j, plate in enumerate(cropped_image):
        print(plate)
    #cv2.imshow("lp", cv2.cvtColor(np.array(cropped_image[0]), cv2.COLOR_RGB2BGR))
    #cv2.waitKey(0)
